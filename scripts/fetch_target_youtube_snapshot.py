    ...
    *,
    ...
    try:
        try:
            ...
            raise RuntimeError(f"target DuckDB snapshot copy failed: {detail}") from None
        if copied.returncode != 0:
            detail = describe_transport_failure(copied.returncode, getattr(copied, "stderr", None))
            raise RuntimeError(f"target DuckDB snapshot copy failed: {detail}")
        try:
            ...
            detail = describe_transport_exception(exc)
            raise RuntimeError(f"target DuckDB snapshot transfer failed: {detail}") from None
        ...
        try:
            ...
            if cleaned.returncode != 0:
                ...
                if cleanup_stderr:
                    ...
                    raise RuntimeError(f"target DuckDB temp cleanup failed: {detail}")
                raise RuntimeError(f"target DuckDB temp cleanup failed rc={cleaned.returncode}")
        except (OSError, subprocess.SubprocessError) as exc:
            if primary_error is None:
                detail = describe_transport_exception(exc)
                raise RuntimeError(f"target DuckDB temp cleanup failed: {detail}") from None
            print(
